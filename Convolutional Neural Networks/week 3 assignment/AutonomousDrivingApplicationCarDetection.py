...
def iou(box1, box2):
    ...
    xi1 = float(max(box1[0], box2[0]))
    yi1 = float(max(box1[1], box2[1]))
    xi2 = float(min(box1[2], box2[2]))
    yi2 = float(min(box1[3], box2[3]))
    inter_width = xi2 - xi1
    ...
    ### START CODE HERE ### (≈ 3 lines)
    box1_area = float((box1[2] - box1[0]) * (box1[3] - box1[1]))
    box2_area = float((box2[2] - box2[0]) * (box2[3] - box2[1]))
...
def yolo_non_max_suppression(scores, boxes, classes, max_boxes=10, iou_threshold=0.5):
    ...
    tf.compat.v1.keras.backend.get_session().run(
        tf.compat.v1.variables_initializer([max_boxes_tensor]))  # initialize variable max_boxes_tensor
...
'''with tf.compat.v1.Session() as test_a:
    ...
    scores, boxes, classes = yolo_filter_boxes(box_confidence, boxes, box_class_probs, threshold=0)
